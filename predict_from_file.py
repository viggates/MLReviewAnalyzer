...
if __name__ == '__main__':
    ...
    dfTickets.shape
    data = dfTickets["Description"]
    data = data.fillna(' ')
    # Loading models
    model_ticket_type = pickle.load(open("outputs/Is_Response.model", "rb"))
    #model_ticket_type = pickle.load(open("outputs/HelpTopic.model", "rb"))
    #description = input()
    import pudb;pu.db
    #predictall(model_ticket_type, data)
    x = model_ticket_type.predict(data)
    y = pd.Series(x, dfTickets['User_ID'])
    y.to_csv('ans.csv',sep="~")
    print(y)
